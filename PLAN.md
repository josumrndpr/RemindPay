...
- [x] **Fase 4 (código)** — PIN Argon2id (crear/pedir/cambiar/bloquear) + ajustes genéricos + respaldos diarios automáticos (VACUUM INTO, últimas 30) + export CSV (pagos/deudas/avisos, Excel-compatible) + tema oscuro/claro persistente + NSIS. 15 tests Rust verificados (cargo test 15/15). Compilación Rust pendiente del workload C++ (nota: el PIN es puerta de acceso, no cifrado del archivo).
